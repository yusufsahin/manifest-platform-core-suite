...
const Sidebar = ({ result, files, activeFile, onOpenFolder, onFileSelect, activeTab, onTabChange, isEmbedded }: SidebarProps) => {
  const menuGroups = [
    {
      group: 'Authoring',
      items: [
            ...
            <div key={section.group}>
              ...
              <div className="space-y-1">
                {section.items.map((item) => (
                  ...
                  >
                    <item.icon className={`w-4 h-4 transition-colors ${activeTab === item.id ? 'text-blue-300' : 'group-hover:text-slate-200'}`} />
