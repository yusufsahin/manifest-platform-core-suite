...
async function loadMPCLibrary(py: PyodideInterface) {
  ...
  // Fetch the manifest generated by sync-mpc.js
  const manifestResp = await fetch('/mpc/manifest.json');
  if (!manifestResp.ok) {
    throw new Error('Failed to load MPC manifest. Ensure sync-mpc.js has run.');
  }
  ...
  const fetchTasks = manifest.map(async (file) => {
    try {
      ...
      const isBinary = file.endsWith('.png') || file.endsWith('.jpg') || file.endsWith('.ico');
      ...
      const absPath = `/home/pyodide/mpc/${file}`;
      if (isBinary) {
        py.FS.writeFile(absPath, new Uint8Array(content as ArrayBuffer));
      } else {
        py.FS.writeFile(absPath, content as string);
      }
    } catch (e) {
  ...
  if (failed.length > 0) {
    console.warn(`Some MPC files failed to load (falling back to partial load): ${failed.slice(0, 5).join(', ')}...`);
  }
...
`);
      ...
      const { data, dsl, context } = payload;
      py.globals.set('DSL', dsl);
      py.globals.set('DATA', JSON.stringify(data));
      py.globals.set('CTX', JSON.stringify(context || {}));
...
import json
from mpc.kernel.parser import parse
from mpc.features.redaction import Redactor
from mpc.kernel.meta.models import DomainMeta, KindDef

ast = parse(DSL)
meta = DomainMeta(kinds=[KindDef(name="Redaction")])
redactor = Redactor(ast=ast, meta=meta)
ctx = json.loads(CTX)
redacted = redactor.redact(json.loads(DATA), context=ctx)

json.dumps({
    "data": redacted
})
`);
        self.postMessage({ id, type: 'REDACTION_RESULT', payload: JSON.parse(result) });
      } finally {
        py.globals.delete('DSL');
        py.globals.delete('DATA');
        py.globals.delete('CTX');
      }
    } else if (type === 'SIMULATE_ACL') {
      const { dsl, role, resource, action } = payload;
      py.globals.set('DSL', dsl);
...
allowed = engine.check_permission(role="${role}", resource="${resource}", action="${action}")

json.dumps({
    "allowed": allowed,
    "context": {"role": "${role}", "resource": "${resource}", "action": "${action}"}
})
`);
        self.postMessage({ id, type: 'ACL_SIM_RESULT', payload: JSON.parse(result) });
      } finally {
        py.globals.delete('DSL');
      }
    }
